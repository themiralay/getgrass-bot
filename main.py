...
# Or if there are multiple tasks
...
def run_client(client_id):
    ...
    # Add the task to the set to keep a strong reference:
    background_tasks.add(task)

    # To prevent the task from being held in memory forever and unable to be garbage collected
    # Let each task remove itself from the set after it finishes:
...
async def delete_client(client_id):
    logger.info(f'[Exit] {all_client[client_id].user_id}')
